...
./server/server
```
3. 이후 새로운 터미널을 켜고 agent를 실행해주세요. 원하는 옵션을 입력해주세요! 옵션은 1개이상 입력해야하고 옵션 1개입력 시 수집주기도 반드시 입력해야합니다.

`현재 소스코드 기준으로 6초간격으로 최대 100회 서버와 연결을 시도합니다. 그전에 서버를 실행해주세요.`

```
./agent/agent -C [수집 주기] -p [수집 주기] -m [수집 주기] -n [수집 주기]
```
